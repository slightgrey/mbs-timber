...
class Timberland extends Timber\Site {
	public function __construct() {
		...
		add_action( 'enqueue_block_editor_assets', array( $this, 'enqueue_editor_autocomplete' ) );
	...
	public function enqueue_assets() {
		...
		if ( $vite_env === 'development' ) {
			...
			// Add editor styles for development mode
			add_editor_style( 'http://localhost:3000/theme/assets/styles/editor-style.css' );
		}
	}

	public function enqueue_editor_autocomplete() {
		wp_enqueue_script(
			'tailwind-autocomplete',
			get_template_directory_uri() . '/assets/editor-autocomplete.js',
			array(),
			filemtime( get_template_directory() . '/assets/editor-autocomplete.js' ),
			true
		);
	}
